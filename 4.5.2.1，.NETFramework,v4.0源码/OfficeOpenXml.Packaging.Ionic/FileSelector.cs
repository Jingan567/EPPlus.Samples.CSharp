...
internal class FileSelector
{
	...
	private static SelectionCriterion _ParseCriterion(string s)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			string text = array[i].ToLower(CultureInfo.InvariantCulture);
			...
			case "or":
				...
				logicalConjunction = (LogicalConjunction)Enum.Parse(typeof(LogicalConjunction), array[i].ToUpper(CultureInfo.InvariantCulture), ignoreCase: true);
			...
			case "size":
			{
				...
				num = (text2.EndsWith("K", StringComparison.OrdinalIgnoreCase) ? (long.Parse(text2.Substring(0, text2.Length - 1)) * 1024) : (text2.EndsWith("KB", StringComparison.OrdinalIgnoreCase) ? (long.Parse(text2.Substring(0, text2.Length - 2)) * 1024) : (text2.EndsWith("M", StringComparison.OrdinalIgnoreCase) ? (long.Parse(text2.Substring(0, text2.Length - 1)) * 1024 * 1024) : (text2.EndsWith("MB", StringComparison.OrdinalIgnoreCase) ? (long.Parse(text2.Substring(0, text2.Length - 2)) * 1024 * 1024) : (text2.EndsWith("G", StringComparison.OrdinalIgnoreCase) ? (long.Parse(text2.Substring(0, text2.Length - 1)) * 1024 * 1024 * 1024) : ((!text2.EndsWith("GB", StringComparison.OrdinalIgnoreCase)) ? long.Parse(array[i + 2]) : (long.Parse(text2.Substring(0, text2.Length - 2)) * 1024 * 1024 * 1024)))))));
