...
public class Floor : ExcelFunction
{
	public override CompileResult Execute(IEnumerable<FunctionArgument> arguments, ParsingContext context)
	{
		...
		if (num2 < 1.0 && num2 > 0.0)
		{
			...
			int num4 = (int)((num - num3) / num2);
			return CreateResult(num3 + (double)num4 * num2, DataType.Decimal);
		}
		if (num2 == 1.0)
		{
			return CreateResult(System.Math.Floor(num), DataType.Decimal);
		}
		if (num % num2 == 0.0)
		{
			return CreateResult(num, DataType.Decimal);
		}
		double num5 = ((!(num > 1.0)) ? (num - num % num2) : (num - num % num2 + num2));
	...
	private void ValidateNumberAndSign(double number, double sign)
	{
		if (number > 0.0 && sign < 0.0)
		{
			...
			throw new InvalidOperationException("Floor cannot handle a negative significance when the number is positive" + text);
		}
	}
}
